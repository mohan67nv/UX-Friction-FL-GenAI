...
Generates production-ready SQL for Supabase PostgreSQL with Row-Level Security (RLS)
"""
...
CREATE TABLE IF NOT EXISTS friction_events (
    ...
    avg_intensity FLOAT,
    page_url_hash TEXT,  -- Hashed URL for privacy
    top_element_hash TEXT,  -- Hashed element selector
    device_type TEXT,
    browser_family TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (hour, project_id, metric_type, COALESCE(page_url_hash, ''), COALESCE(top_element_hash, ''))
);
...
def main():
    """Generate and save Supabase schema SQL"""
    output_file = "supabase_schema.sql"
    
    with open(output_file, "w") as f:
        f.write(SUPABASE_SCHEMA_SQL)
    
    print("✅ Supabase schema generated successfully!")
    print(f"📄 File: {output_file}")
    print(f"📏 Size: {len(SUPABASE_SCHEMA_SQL)} characters")
    print("\n📋 Next steps:")
    print("1. Open Supabase Dashboard: https://supabase.com/dashboard/project/egwjnhrxwmorcwnvjzja/sql")
    print("2. Copy contents of supabase_schema.sql")
    print("3. Paste into SQL Editor")
    print("4. Click 'Run'")
    print("5. Verify tables in Table Editor")
    print("\n🔍 Schema includes:")
    print("   ✅ 8 tables (users, organizations, projects, api_keys, friction_events, etc.)")
    print("   ✅ Row-Level Security (RLS) policies")
    print("   ✅ Indexes for performance")
    print("   ✅ Triggers for auto-updating timestamps")
    print("   ✅ Helper functions for user permissions")
