# 🚀 Quick Start Guide - ZeroBanner MVP Demo
...
**Expected Output:**
```
✔ Container zerobanner-fl-genai-postgres-1    Started
✔ Container zerobanner-fl-genai-redis-1       Started
✔ Container zerobanner-fl-genai-qdrant-1      Started
✔ Container zerobanner-fl-genai-api-1         Started
✔ Container zerobanner-fl-genai-dashboard-1   Started
```
...
```bash
docker ps --format "table {{.Names}}\t{{.Status}}" | grep zerobanner
```
...
|---------|------|---------|--------------|
| **PostgreSQL** | 5433 | Analytics storage | `docker logs zerobanner-fl-genai-postgres-1` |
| **Redis** | 6380 | Rate limiting | `docker logs zerobanner-fl-genai-redis-1` |
...
# Verify files exist
docker exec zerobanner-fl-genai-api-1 ls -lh /app/static/
```
...
```bash
# Seed demo data
docker exec zerobanner-fl-genai-api-1 python scripts/seed_demo_data.py
```
...
### **Opening (30 seconds)**
> "I built ZeroBanner - a privacy-first UX analytics platform using Federated Learning. Let me show you the complete system running locally."
...
1. **SDK:** Show [client/src/index.ts](client/src/index.ts) - ZeroBannerAnalytics class
...
│  │                                                       │  │
│  │  Loads: ZeroBannerAnalytics SDK ─────────────┐     │  │
│  │  From:  http://localhost:8001/static/client.js │    │  │
│  └────────────────────────────────────────────────┼─────┘  │
│                                                    │        │
│  ┌──────────────────────────────────────────────┐ │        │
│  │  ZeroBanner SDK (Local Training)            │ │        │
...
```bash
# API logs
docker logs zerobanner-fl-genai-api-1

# Dashboard logs
docker logs zerobanner-fl-genai-dashboard-1

# Database logs
docker logs zerobanner-fl-genai-postgres-1
```
